...
class Algo():
    ...
    def send(self):
        if self.outbox != '' and self.sent == False:
            self.sent = True
            return self.outbox
	    
    def read(self, fro):
        while not self.inbox or not (fro in string for string in self.inbox):
            pass
        for i in range(len(self.inbox), -1, -1):
            if fro in self.inbox[i]:
##              might need more processing here                
                return self.inbox.pop(i).split('|')[1]
...
try:
    rdata = mail.read('AND')
    robot = Robot([rdata[0], rdata[1]],rdata[2])
except:
    print("robot initialization failed")
    exit()
try:
    rdata = mail.read('AND')
    waypoint = [rdata[0], rdata[1]]
except:
...
def robot_sense(robot):
    ...
    mail.write('AND|'+ print_descriptor() + ';' + robot.coord + ';' + robot.face)
    while not mail.sent():
        pass
