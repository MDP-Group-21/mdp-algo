...
def robot_sense(robot):
    ...
    if 'UP' in sensedata:
        ...
        if robot.face == 'W':
            if x+2 in range(15) and y+1 in range(20):
                mdp_map[x+2][y+1].has_image = True
                mdp_map[x+2][y+1].imgdir = 'A'
                imgstr = str(x+2) + ';' + str(y+1) + ';' + 'A'
        elif robot.face == 'A':
            if y+2 in range(20) and x-1 in range(15):
                mdp_map[x-1][y+2].has_image = True
                mdp_map[x-1][y+2].imgdir = 'S'
                imgstr = str(x-1) + ';' + str(y+2) + ';' + 'S'
        elif robot.face == 'S':
            if x-2 in range(15) and y-1 in range(20):
                mdp_map[x-2][y-1].has_image = True
                mdp_map[x-2][y-1].imgdir = 'D'
                imgstr = str(x-2) + ';' + str(y-1) + ';' + 'D'
        elif robot.face == 'D':
            if y-2 in range(20) and x+1 in range(15):
                mdp_map[x+1][y-2].has_image = True
                mdp_map[x+1][y-2].imgdir = 'W'
                imgstr = str(x+1) + ';' + str(y-2) + ';' + 'W'
...
def explore(robot):
    ...
    while robot.coord != start or (robot.coord == start and goal_explored == False) or (goal_explored == False and start_explored == False) or not robot.killswitch:
##        if steps == time_limit:
##            print('Time up!')
...
def fastest_path(robot, begin, end):
    ...
    while eye != end:
        ...
        if mdp_map[eye[1]][eye[0]].fpath == 'W':
            ...
            eye[0] += 1
    #print(robot.coord)
    #Need to convert multiple consecutives into a single command
    if robot.coord != begin:
        print("Where's my robot?")
    else:
        while robot.coord != end or not killswitch:
            next_path = path[0]
