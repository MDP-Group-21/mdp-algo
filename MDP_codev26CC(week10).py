...
def h_eval(x, y, h):
    ###
    ### TO-DO:  improve heuristics: to prefer lesser predicted turns,
    ###         min of min(x_distance, y_distance)
    ###
    hor = abs(x-h[0])
    ver = abs(y-h[1])
    if hor <= ver:
...
##            fastest_path(robot, robot.coord, start)
##            return
        if goal_explored == True and start_explored == True and robot.coord != start:
...
##        print(steps)
        ...
        if robot.face == 'W':
            if mdp_map[robot.coord[1]+1][robot.coord[0]+2].text == 'W':
                cruise(robot)
        elif robot.face == 'A':
            if mdp_map[robot.coord[1]+2][robot.coord[0]-1].text == 'W':
                cruise(robot)
        elif robot.face == 'S':
            if mdp_map[robot.coord[1]-1][robot.coord[0]-2].text == 'W':
                cruise(robot)
