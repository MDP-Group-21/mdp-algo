import numpy as np
import sys
import webbrowser, os
import socket
from operator import attrgetter
from testsocket import Algo
import time
import math

try: color = sys.stdout.shell
except AttributeError: raise RuntimeError("Use IDLE")

class Robot(object):
    def __init__(self, coord, face, wall_left=False, wall_right=False, wall_ahead=False, second_ahead=False, movetwo = False, imgstr = ''):
        self.coord = coord
        self.face = face
        self.wall_left = wall_left
        self.wall_right = wall_right
        self.wall_ahead = wall_ahead
        self.second_ahead = second_ahead
        self.movetwo = movetwo
        self.killswitch = False
        self.imgstr = imgstr
    def __getitem__(self,index):
        if index == 0:
            return self.coord
        elif index == 1:
            return self.face
    def __setitem__(self,index,value):
        self[index] = value
class Coordinates(object):
    def __init__(self, coord):
        self.x = coord[0]
        self.y = coord[1]
    def __getitem__(self,index):
        if index == 0:
            return self.x
        elif index == 1:
            return self.y
class Cell(object):
    def __init__(self, text, explored = False, has_image = False, imgdir = '', fpath = '1', fval = 0):
        self.text = text
        self.explored = explored
        self.has_image = has_image
        self.imgdir = imgdir
        self.fpath = fpath
        self.fval = fval
class Frontier(object):
    def __init__(self, coord, step, hvalue):
        self.coord = coord
        self.x = coord[0]
        self.y = coord[1]
        self.step = step
        self.hvalue = hvalue
        self.total = step + hvalue


#2 parts to sending data
#1. 0 for unexplored, 1 for explored
#2. (only for explored cell) 0 for empty, 1 for obstacle

def initialize_map():
    if os.path.isfile("realtimemap.txt"):
        os.remove("realtimemap.txt")
##    if os.path.isfile("explore.txt"):
##        os.remove("explore.txt")
    mdp_map_row = []
    mdp_explore_map_row = []
    for j in range(20):
        for i in range(15):
            if i in range(start[0]+2) and j in range(start[1]+2):
                mdp_map_row.append(Cell('S'))
            elif i in range(goal[0]-1, goal[0]+2) and j in range(goal[1]-1,goal[1]+2):
                mdp_map_row.append(Cell('G'))
            elif i == 0 or i == 14 or j == 0 or j == 19:
                mdp_map_row.append(Cell('V'))
            else:
                mdp_map_row.append(Cell(' '))
            mdp_explore_map_row.append(Cell(' '))
        mdp_explore_map.append(mdp_explore_map_row)
        mdp_map.append(mdp_map_row)
        mdp_map_row = []
    
def reset_fpath():
    for j in range(20):
        for i in range(15):
            mdp_map[j][i].fpath = '1'
            mdp_map[j][i].fval = 0

def fully_explored():
    for j in range(20):
        for i in range(15):
            if not mdp_map[j][i].explored:
                return False
    return True

def valid_space(x, y):
    for j in range(3):
        for i in range(3):
            if mdp_map[y-1+j][x-1+i].text == 'W' or not mdp_map[y-1+j][x-1+i].explored:
                return False
    return True

def to_explore(fro):
    candidate = []
    distance = 999999
    evdistance = 0
    for j in range(19,-1,-1):
        for i in range(15):
            if not mdp_map[j][i].explored:
                if mdp_map[j][i-1].explored and mdp_map[j][i-2].explored and mdp_map[j][i-2].text != 'W' and mdp_map[j][i-1].text != 'W':
                    if valid_space(i-2, j):
                        evdistance = ((i-2) - fro[0]) + 2*(j - fro[1])
                        if evdistance < distance:
                            candidate = [i-2, j]
                            if (i-2) - fro[0] == 0 or j - fro[1] == 0:
                                return candidate
                            distance = evdistance
                if mdp_map[j-1][i].explored and mdp_map[j-2][i].explored and mdp_map[j-2][i].text != 'W' and mdp_map[j-1][i].text != 'W':
                    if valid_space(i, j-2):
                        evdistance = (i - fro[0]) + 2*((j-2) - fro[1])
                        if evdistance < distance:
                            candidate = [i, j-2]
                            if i - fro[0] == 0 or (j-2) - fro[1] == 0:
                                return candidate
                            distance = evdistance
                if mdp_map[j][i+1].explored and mdp_map[j][i+2].explored and mdp_map[j][i+2].text != 'W' and mdp_map[j][i+1].text != 'W':
                    if valid_space(i+2, j):
                        evdistance = ((i+2) - fro[0]) + 2*(j - fro[1])
                        if evdistance < distance:
                            candidate = [i+2, j]
                            if (i+2) - fro[0] == 0 or j - fro[1] == 0:
                                return candidate
                            distance = evdistance
                if mdp_map[j+1][i].explored and mdp_map[j+2][i].explored and mdp_map[j+2][i].text != 'W' and mdp_map[j+1][i].text != 'W':
                    if valid_space(i, j+2):
                        evdistance = (i - fro[0]) + 2*((j+2) - fro[1])
                        if evdistance < distance:
                            candidate = [i, j+2]
                            if i - fro[0] == 0 or (j+2) - fro[1] == 0:
                                return candidate
                            distance = evdistance
##    print('candidate: ' + str(candidate))
    return candidate

def update_map(xcoord, ycoord, text):
    mdp_map[ycoord][xcoord].text = text

def burst_mode(text):
    wstring = 'WWWWWWWWWW'
    rstring = ''
    for i in range(10,1,-1):
        if i == 17:
            rstring = 'M'
        elif i == 16:
            rstring = 'L'
        elif i == 15:
            rstring = 'K'
        elif i == 14:
            rstring = 'J'
        elif i == 13:
            rstring = 'H'
        elif i == 12:
            rstring = 'G'
        elif i == 11:
            rstring = 'F'
        elif i == 10:
            rstring = 'Q'
        elif i == 9:
            rstring = 'P'
        elif i == 8:
            rstring = 'O'
        elif i == 7:
            rstring = 'I'
        elif i == 6:
            rstring = 'U'
        elif i == 5:
            rstring = 'Y'
        elif i == 4:
            rstring = 'T'
        elif i == 3:
            rstring = 'R'
        else:
            rstring = 'E'
        text = text.replace(wstring[:i],rstring)
    return text

    
def print_map():
    for j in range(19,-1,-1):
        for i in range(15):
            if mdp_map[j][i].text == 'R':
                color.write('[' + mdp_map[j][i].text + ']', 'STRING') #green
            elif mdp_map[j][i].text == 'H':
                color.write('[' + mdp_map[j][i].text + ']', 'KEYWORD') #orange
            elif mdp_map[j][i].has_image:
                color.write('[' + mdp_map[j][i].imgdir + ']', 'COMMENT')
            else:
                print('[' + mdp_map[j][i].text + ']', end='')
        print()
    print()

def print_fpath():
    for j in range(19,-1,-1):
        for i in range(15):
            print('['+mdp_map[j][i].fpath+']',end='')
        print()
    print()

def oob(x,y):
    return (x not in range(15) or y not in range(20))

##def print_explored():
##    for j in range(19,-1,-1):
##        for i in range(15):
##            if mdp_map[j][i].explored:
##                print('[1]', end='')
##            else:
##                print('[0]', end='')
##        print()
##    print()

def explore_percent():
    var1 = 0
    for j in range(20):
        for i in range(15):
            if mdp_map[j][i].explored:
                var1 += 1
    return int((var1/300)*100)

def print_descriptor():
    descriptor_1 = ['11']
    descriptor_2 = []
    for j in range(20):
        for i in range(15):
            if mdp_map[j][i].explored:
                descriptor_1.append('1')
                if mdp_map[j][i].text == 'W':
                    descriptor_2.append('1')
                else:
                    descriptor_2.append('0')
            else:
                descriptor_1.append('0')
    descriptor_1.append('11')

    des1 = ''.join(descriptor_1)
    hstr = '%0*X' % ((len(des1) + 3) // 4, int(des1, 2))
    #print(hstr)
    des2 = ''.join(descriptor_2)
    while len(des2) % 8 != 0:
        des2 += '0'
    hstr2 = '%0*X' % ((len(des2) + 3) // 4, int(des2, 2))
    #print(hstr2)
    return hstr + ';' + hstr2

def print_alt_descriptor():
    descriptor = []
    for j in range(19,-1,-1):
        for i in range(15):
            if mdp_map[j][i].explored:
                if mdp_map[j][i].text == ' ' or mdp_map[j][i].text == 'V':
                    descriptor.append('S')
                else:
                    descriptor.append(mdp_map[j][i].text)
            else:
                descriptor.append('X')
    des = ''.join(descriptor)
    #print(des)
    return des + '\n'

def place_wall(x_coord, y_coord):
    if x_coord in range(15) and y_coord in range(20):
        mdp_map[y_coord][x_coord].text = 'W'
        place_vwall(x_coord+1, y_coord)
        place_vwall(x_coord, y_coord+1)
        place_vwall(x_coord-1, y_coord)
        place_vwall(x_coord, y_coord-1)
    else:
        print('Wall out of range')
        
def place_vwall(x_coord, y_coord):
    if x_coord in range(15) and y_coord in range(20) and mdp_map[y_coord][x_coord].text!='W':
        mdp_map[y_coord][x_coord].text = 'V'

def and_write(robot):
    if robot.imgstr == '':
        mail.write('AND|'+ print_descriptor() + ';' + str(robot.coord[0]) + ';' + str(robot.coord[1]) + ';' + robot.face)
    else:
        #print('imgstr: ',end='')
        print(robot.imgstr)
        mail.write('AND|'+ print_descriptor() + ';' + str(robot.coord[0]) + ';' + str(robot.coord[1]) + ';' + robot.face + ';' + robot.imgstr)

def place_robot(robot_coord):
    x = robot_coord[0]
    y = robot_coord[1]
    place = True
    for i in range(3):
        for j in range(3):
            if mdp_map[y-1+j][x-1+i].text == 'W':
                #place = False
                place = True
    if place:
        for i in range(3):
            for j in range(3):
                update_map(x-1+i, y-1+j, 'R')
                mdp_map[y-1+j][x-1+i].explored = True
        if robot.face == 'W':
            update_map(x, y+1, 'H')
        elif robot.face == 'A':
            update_map(x-1, y, 'H')
        elif robot.face == 'S':
            update_map(x, y-1, 'H')
        elif robot.face == 'D':
            update_map(x+1, y, 'H')
##        robot_sense(robot)
        with open("realtimemap.txt","w") as text_file_sim:
            text_file_sim.write(print_alt_descriptor())
    else:
        print('Error in robot placement.')

def robot_cornered(robot):
##    print('robot corner check')
    x = robot.coord[0]
    y = robot.coord[1]
    if robot.face == 'W':
        if x+2 not in range(15) and y+2 not in range(20):
            return True
        elif x+2 in range(15) and y+2 in range(20):
            if mdp_map[y-1][x+2].text == 'W' and mdp_map[y][x+2].text == 'W' and mdp_map[y+1][x+2].text == 'W' and mdp_map[y+2][x-1].text == 'W' and mdp_map[y+2][x].text == 'W' and mdp_map[y+2][x+1].text == 'W':
                return True
    elif robot.face == 'A':
        if x-2 not in range(15) and y+2 not in range(20):
            return True
        elif x-2 in range(15) and y+2 in range(20):
            if mdp_map[y-1][x-2].text == 'W' and mdp_map[y][x-2].text == 'W' and mdp_map[y+1][x-2].text == 'W' and mdp_map[y+2][x-1].text == 'W' and mdp_map[y+2][x].text == 'W' and mdp_map[y+2][x+1].text == 'W':
                return True
    elif robot.face == 'S':
        if x-2 not in range(15) and y-2 not in range(20):
            return True
        elif x-2 in range(15) and y-2 in range(20):
            if mdp_map[y-1][x-2].text == 'W' and mdp_map[y][x-2].text == 'W' and mdp_map[y+1][x-2].text == 'W' and mdp_map[y-2][x-1].text == 'W' and mdp_map[y-2][x].text == 'W' and mdp_map[y-2][x+1].text == 'W':
                return True
    elif robot.face == 'D':
        if x+2 not in range(15) and y-2 not in range(20):
            return True
        elif x+2 in range(15) and y-2 in range(20):
            if mdp_map[y-1][x+2].text == 'W' and mdp_map[y][x+2].text == 'W' and mdp_map[y+1][x+2].text == 'W' and mdp_map[y-2][x-1].text == 'W' and mdp_map[y-2][x].text == 'W' and mdp_map[y-2][x+1].text == 'W':
                return True
def robot_rightwall(robot):
    x = robot.coord[0]
    y = robot.coord[1]
    if robot.face == 'W':
        if x+2 not in range(15):
            return True
        elif mdp_map[y-1][x+2].text == 'W' and mdp_map[y][x+2].text == 'W' and mdp_map[y+1][x+2].text == 'W':
            return True
    elif robot.face == 'A':
        if y+2 not in range(20):
            return True
        elif mdp_map[y+2][x-1].text == 'W' and mdp_map[y+2][x].text == 'W' and mdp_map[y+2][x+1].text == 'W':
            return True
    elif robot.face == 'S':
        if x-2 not in range(15):
            return True
        elif mdp_map[y-1][x-2].text == 'W' and mdp_map[y][x-2].text == 'W' and mdp_map[y+1][x-2].text == 'W':
            return True
    elif robot.face == 'D':
        if y-2 not in range(20):
            return True
        elif mdp_map[y-2][x-1].text == 'W' and mdp_map[y-2][x].text == 'W' and mdp_map[y-2][x+1].text == 'W':
            return True
        
def robot_sense(robot, msg, up=False):
    #imgdata: check for RPI in sensedata
    #based on cam position
    #stick on nearest relevant block
    sflag = False
    msg = str(msg)
    #mail.write('ARD|.')
    #sensedata = mail.read('ARD')
    #if 'ACK' in sensedata:
    while not sflag:
        try:
            #print('robot sense msg: ',end='')
            #print(msg)
            print('robot coord: ',end='')
            print(robot.coord)
            if up:
                sensedata = list(map(int, msg[0:6]))
            else:
                sensedata = list(map(int, msg))
            print('robot sense sensedata: ',end='')
            print(sensedata)
            #crashed somewhere here(clue, kept crashing)
            if any(i in sensedata for i in illegals):
                sflag = False
                color.write('Sensor data ambiguous, rereading...','KEYWORD')
                time.sleep(0.5)
                mail.write('ARD|.')
                msg = mail.read('ARD')
            else:
                sflag = True
        except:
            color.write('Sensor data error, rereading...','COMMENT')
            mail.write('ARD|.')
            msg = mail.read('ARD')

    #print('SENSOR DATA READ')
    robot_coord = robot[0]
    x = robot_coord[0]
    y = robot_coord[1]
    robot.wall_left = False
    robot.wall_right = False
    robot.wall_ahead = False
    robot.second_ahead = False
    robot.movetwo = False

    #[ ][ ][ ]
    #[ ][ ][ ]
    #[ ][ ][ ]
    #initialize sensing zone
    if robot.face == 'W':
        if y+2 in range(20):
            pt1 = mdp_map[y+2][x-1]
            pt4 = mdp_map[y+2][x]
            pt7 = mdp_map[y+2][x+1]
        else:
            pt1 = pt4 = pt7 = 'x'
        if y+3 in range(20):
            pt2 = mdp_map[y+3][x-1]
            pt5 = mdp_map[y+3][x]
            pt8 = mdp_map[y+3][x+1]
        else:
            pt2 = pt5 = pt8 = 'x'
        if y+4 in range(20):
            pt3 = mdp_map[y+4][x-1]
            pt6 = mdp_map[y+4][x]
            pt9 = mdp_map[y+4][x+1]
        else:
            pt3 = pt6 = pt9 = 'x'
        if x+2 in range(15):
            pt10 = mdp_map[y+1][x+2]
            pt10a = mdp_map[y][x+2]
            pt10b = mdp_map[y-1][x+2]
        else:
            pt10 = pt10a = pt10b = 'x'
        if x+3 in range(15):
            pt11 = mdp_map[y+1][x+3]
        else:
            pt11 = 'x'
        if x-2 in range(15):
            pt12 = mdp_map[y+1][x-2]
            pt12a = mdp_map[y][x-2]
            pt12b = mdp_map[y-1][x-2]
        else:
            pt12 = pt12a = pt12b = 'x'
        if x-3 in range(15):
            pt13 = mdp_map[y+1][x-3]
        else:
            pt13 = 'x'
        if x+2 in range(15) and y+2 in range(20):
            corner = mdp_map[y+2][x+2]
        else:
            corner = 'x'
    elif robot.face == 'A':
        if x-2 in range(15):
            pt1 = mdp_map[y-1][x-2]
            pt4 = mdp_map[y][x-2]
            pt7 = mdp_map[y+1][x-2]
        else:
            pt1 = pt4 = pt7 = 'x'
        if x-3 in range(15):
            pt2 = mdp_map[y-1][x-3]
            pt5 = mdp_map[y][x-3]
            pt8 = mdp_map[y+1][x-3]
        else:
            pt2 = pt5 = pt8 = 'x'
        if x-4 in range(15):
            pt3 = mdp_map[y-1][x-4]
            pt6 = mdp_map[y][x-4]
            pt9 = mdp_map[y+1][x-4]
        else:
            pt3 = pt6 = pt9 = 'x'
        if y+2 in range(20):
            pt10 = mdp_map[y+2][x-1]
            pt10a = mdp_map[y+2][x]
            pt10b = mdp_map[y+2][x+1]
        else:
            pt10 = pt10a = pt10b = 'x'
        if y+3 in range(20):
            pt11 = mdp_map[y+3][x-1]
        else:
            pt11 = 'x'
        if y-2 in range(20):
            pt12 = mdp_map[y-2][x-1]
            pt12a = mdp_map[y-2][x]
            pt12b = mdp_map[y-2][x+1]
        else:
            pt12 = pt12a = pt12b = 'x'
        if y-3 in range(20):
            pt13 = mdp_map[y-3][x-1]
        else:
            pt13 = 'x'
        if x-2 in range(15) and y+2 in range(20):
            corner = mdp_map[y+2][x-2]
        else:
            corner = 'x'
    elif robot.face == 'S':
        if y-2 in range(20):
            pt1 = mdp_map[y-2][x+1]
            pt4 = mdp_map[y-2][x]
            pt7 = mdp_map[y-2][x-1]
        else:
            pt1 = pt4 = pt7 = 'x'
        if y-3 in range(20):
            pt2 = mdp_map[y-3][x+1]
            pt5 = mdp_map[y-3][x]
            pt8 = mdp_map[y-3][x-1]
        else:
            pt2 = pt5 = pt8 = 'x'
        if y-4 in range(20):
            pt3 = mdp_map[y-4][x+1]
            pt6 = mdp_map[y-4][x]
            pt9 = mdp_map[y-4][x-1]
        else:
            pt3 = pt6 = pt9 = 'x'
        if x-2 in range(15):
            pt10 = mdp_map[y-1][x-2]
            pt10a = mdp_map[y][x-2]
            pt10b = mdp_map[y+1][x-2]
        else:
            pt10 = pt10a = pt10b = 'x'
        if x-3 in range(15):
            pt11 = mdp_map[y-1][x-3]
        else:
            pt11 = 'x'
        if x+2 in range(15):
            pt12 = mdp_map[y-1][x+2]
            pt12a = mdp_map[y][x+2]
            pt12b = mdp_map[y+1][x+2]
        else:
            pt12 = pt12a = pt12b = 'x'
        if x+3 in range(15):
            pt13 = mdp_map[y-1][x+3]
        else:
            pt13 = 'x'
        if x-2 in range(15) and y-2 in range(20):
            corner = mdp_map[y-2][x-2]
        else:
            corner = 'x'
    elif robot.face == 'D':
        if x+2 in range(15):
            pt1 = mdp_map[y+1][x+2]
            pt4 = mdp_map[y][x+2]
            pt7 = mdp_map[y-1][x+2]
        else:
            pt1 = pt4 = pt7 = 'x'
        if x+3 in range(15):
            pt2 = mdp_map[y+1][x+3]
            pt5 = mdp_map[y][x+3]
            pt8 = mdp_map[y-1][x+3]
        else:
            pt2 = pt5 = pt8 = 'x'
        if x+4 in range(15):
            pt3 = mdp_map[y+1][x+4]
            pt6 = mdp_map[y][x+4]
            pt9 = mdp_map[y-1][x+4]
        else:
            pt3 = pt6 = pt9 = 'x'
        if y-2 in range(20):
            pt10 = mdp_map[y-2][x+1]
            pt10a = mdp_map[y-2][x]
            pt10b = mdp_map[y-2][x-1]
        else:
            pt10 = pt10a = pt10b = 'x'
        if y-3 in range(20):
            pt11 = mdp_map[y-3][x+1]
        else:
            pt11 = 'x'
        if y+2 in range(20):
            pt12 = mdp_map[y+2][x+1]
            pt12a = mdp_map[y+2][x]
            pt12b = mdp_map[y+2][x-1]
        else:
            pt12 = pt12a = pt12b = 'x'
        if y+3 in range(20):
            pt13 = mdp_map[y+3][x+1]
        else:
            pt13 = 'x'
        if x+2 in range(15) and y-2 in range(20):
            corner = mdp_map[y-2][x+2]
        else:
            corner = 'x'
        

    if up:
        #sensedata = sensedata[0]
        color.write('image detected', 'STRING')
        x = robot.coord[0]
        y = robot.coord[1]
        #image placement code
        for i in range(2, 5):
            if robot.face == 'W':
                print('W')
                if x+i in range(15) and mdp_map[y][x+i].text == 'W':
                    mdp_map[y][x+i].has_image = True
                    mdp_map[y][x+i].imgdir = 'A'
                    robot.imgstr = str(x+i) + ';' + str(y) + ';' + 'A'
                    print(robot.imgstr)
                    break
            elif robot.face == 'A':
                print('A')
                if y+i in range(20) and mdp_map[y+i][x].text == 'W':
                    mdp_map[y+i][x].has_image = True
                    mdp_map[y+i][x].imgdir = 'S'
                    robot.imgstr = str(x) + ';' + str(y+i) + ';' + 'S'
                    print(robot.imgstr)
                    break
            elif robot.face == 'S':
                print('S')
                if x-i in range(15) and mdp_map[y][x-i].text == 'W':
                    mdp_map[y][x-i].has_image = True
                    mdp_map[y][x-i].imgdir = 'D'
                    robot.imgstr = str(x-i) + ';' + str(y) + ';' + 'D'
                    print(robot.imgstr)
                    break
            elif robot.face == 'D':
                print('D')
                if y-i in range(20) and mdp_map[y-i][x].text == 'W':
                    mdp_map[y-i][x].has_image = True
                    mdp_map[y-i][x].imgdir = 'W'
                    robot.imgstr = str(x) + ';' + str(y-i) + ';' + 'W'
                    print(robot.imgstr)
                    break
    else:
        robot.imgstr = ''

    #ARD sense data: 0 - immediate, 1 - 1 away, 2 - 2 away, 9 - no obstacles

    #firstsense
##    try:
##        if sensedata[0] == 9:
##            pt1.explored = pt2.explored = pt3.explored = True
##            pt1.text = pt2.text = pt3.text = ' '
##        elif sensedata[0] == 2:
##            pt1.explored = pt2.explored = True
##            pt1.text = pt2.text = ' '
##            if pt3 != 'x':
##                pt3.explored = True
##                pt3.text = 'W'
##        elif sensedata[0] == 1:
##            pt1.explored = True
##            pt1.text = ' '
##            if pt2 != 'x':
##                pt2.explored = True
##                pt2.text = 'W'
##        elif sensedata[0] == 0:
##            robot.wall_ahead = True
##            if pt1 != 'x':
##                pt1.explored = True
##                pt1.text = 'W'
##    except:
##        color.write('Front sensor 1 input error','KEYWORD')
##            
##
##    try:        
##        if sensedata[1] == 9:
##            pt4.explored = pt5.explored = pt6.explored = True
##            pt4.text = pt5.text = pt6.text = ' '
##        elif sensedata[1] == 2:
##            pt4.explored = pt5.explored = True
##            pt4.text = pt5.text = ' '
##            if pt6 != 'x':
##                pt6.explored = True
##                pt6.text = 'W'
##        elif sensedata[1] == 1:
##            pt4.explored = True
##            pt4.text = ' '
##            if pt5 != 'x':
##                pt5.explored = True
##                pt5.text = 'W'
##        elif sensedata[1] == 0:
##            robot.wall_ahead = True
##            if pt4 != 'x':
##                pt4.explored = True
##                pt4.text = 'W'
##    except:
##        color.write('Front sensor 2 input error','KEYWORD')
##            
##
##    try:    
##        if sensedata[2] == 9:
##            pt7.explored = pt8.explored = pt9.explored = True
##            pt7.text = pt8.text = pt9.text = ' '
##        elif sensedata[2] == 2:
##            pt7.explored = pt8.explored = True
##            pt7.text = pt8.text = ' '
##            if pt9 != 'x':
##                pt9.explored = True
##                pt9.text = 'W'
##        elif sensedata[2] == 1:
##            pt7.explored = True
##            pt7.text = ' '
##            if pt8 != 'x':
##                pt8.explored = True
##                pt8.text = 'W'
##        elif sensedata[2] == 0:
##            robot.wall_ahead = True
##            if pt7 != 'x':
##                pt7.explored = True
##                pt7.text = 'W'
##    except:
##        color.write('Front sensor 3 input error','KEYWORD')
##        
##    try:
##        if sensedata[3] == 9:
##            pt10.explored = pt11.explored = True
##            pt10.text = pt11.text = ' '
##        elif sensedata[3] == 1:
##            pt10.explored = True
##            pt10.text = ' '
##            if pt11 != 'x':
##                pt11.explored = True
##                pt11.text = 'W'
##        elif sensedata[3] == 0:
##            robot.wall_right = True
##            robot.right_decay = 2
##            if pt10 != 'x':
##                pt10.explored = True
##                pt10.text = 'W'
##    except:
##        color.write('Right sensor input error','KEYWORD')
##
##    try:
##        if sensedata[4] == 9:
##            pt12.explored = pt13.explored = True
##            pt12.text = pt13.text = ' '
##        elif sensedata[4] == 1:
##            pt12.explored = True
##            pt12.text = ' '
##            if pt13 != 'x':
##                pt13.explored = True
##                pt13.text = 'W'
##        elif sensedata[4] == 0:
##            robot.wall_left = True
##            if pt12 != 'x':
##                pt12.explored = True
##                pt12.text = 'W'
##    except:
##        color.write('Left sensor input error','KEYWORD')

    try:
        if sensedata[0] == 9 or sensedata[0] == 2:
            pt1.explored = pt2.explored = True
            pt1.text = pt2.text = ' '
        elif sensedata[0] == 1:
            pt1.explored = True
            pt1.text = ' '
            if pt2 != 'x':
                pt2.explored = True
                pt2.text = 'W'
        elif sensedata[0] == 0:
            if pt1 != 'x':
                pt1.explored = True
                pt1.text = 'W'
    except:
        color.write('Front sensor 1 input error','KEYWORD')

    try:
        if sensedata[1] == 9 or sensedata[1] == 2:
            pt4.explored = pt5.explored = True
            pt4.text = pt5.text = ' '
        elif sensedata[1] == 1:
            pt4.explored = True
            pt4.text = ' '
            if pt5 != 'x':
                pt5.explored = True
                pt5.text = 'W'
        elif sensedata[1] == 0:
            if pt4 != 'x':
                pt4.explored = True
                pt4.text = 'W'
    except:
        color.write('Front sensor 2 input error','KEYWORD')

    try:
        if sensedata[2] == 9 or sensedata[2] == 2:
            pt7.explored = pt8.explored = True
            pt8.text = pt8.text = ' '
        elif sensedata[2] == 1:
            pt7.explored = True
            pt7.text = ' '
            if pt8 != 'x':
                pt8.explored = True
                pt8.text = 'W'
        elif sensedata[2] == 0:
            if pt7 != 'x':
                pt7.explored = True
                pt7.text = 'W'
    except:
        color.write('Front sensor 3 input error','KEYWORD')
                
    #altsense
    #debug
##    try:
##        if sensedata[0] == 1 or sensedata[0] == 9:
##            if pt1 != 'x':
##                pt1.explored = True
##                pt1.text = ' '
##        elif sensedata[0] == 0:
##            robot.wall_ahead = True
##            if pt1 != 'x':
##                pt1.explored = True
##                pt1.text = 'W'
##    except:
##        color.write('Front sensor 1 input error','KEYWORD')
##            
##
##    try:        
##        if sensedata[1] == 1 or sensedata[1] == 9:
##            if pt4 != 'x':
##                pt4.explored = True
##                pt4.text = ' '
##        elif sensedata[1] == 0:
##            robot.wall_ahead = True
##            if pt4 != 'x':
##                pt4.explored = True
##                pt4.text = 'W'
##    except:
##        color.write('Front sensor 2 input error','KEYWORD')
##            
##
##    try:    
##        if sensedata[2] == 1 or sensedata[2] == 9:
##            if pt7 != 'x':
##                pt7.explored = True
##                pt7.text = ' '
##        elif sensedata[2] == 0:
##            robot.wall_ahead = True
##            if pt7 != 'x':
##                pt7.explored = True
##                pt7.text = 'W'
##    except:
##        color.write('Front sensor 3 input error','KEYWORD')
        
    try:
        if sensedata[3] == 1 or sensedata[3] == 9:
            if pt10 != 'x':
                pt10.explored = True
                pt10.text = ' '
        elif sensedata[3] == 0:
            #print('wallright')
            if pt10 != 'x':
                pt10.explored = True
                pt10.text = 'W'
    except:
        color.write('Right sensor input error','KEYWORD')

    try:
        if sensedata[4] == 1 or sensedata[4] == 9:
            if pt12 != 'x':
                pt12.explored = True
                pt12.text = ' '
        elif sensedata[4] == 0:
            #print('wallleft')
            if pt12 != 'x':
                pt12.explored = True
                pt12.text = 'W'
    except:
        color.write('Left sensor input error','KEYWORD')


    #Evaluate wall ahead, left, right, and second_ahead
    if (pt1 == 'x') or (pt1.text == 'W' or pt4.text == 'W' or pt7.text == 'W'):
        robot.wall_ahead = True
        #print('wallahead')
    if (pt10 == 'x') or (pt10.text == 'W' or pt10a.text == 'W' or pt10b.text == 'W'):
        robot.wall_right = True
        #print('wallright')
    #if pt10 == 'x' or corner.text == 'W':
    if pt10 == 'x':
        #print('MOVING TWO NEXT')
        robot.movetwo = True
        if (pt10 == 'x') or (pt10.text == 'W' or pt10a.text == 'W' or pt10b.text == 'W'):
            #print('wallright')
            robot.wall_right = True
    if (pt2 == 'x') or (pt2.text == 'W' or pt5.text == 'W' or pt8.text == 'W'):
        #print('secondahead')
        robot.second_ahead = True

    #Wipe walls from both zones    
    for j in range(3):
        for i in range(3):
            mdp_map[start[1]-1+j][start[0]-1+i].text = 'S'
            mdp_map[goal[1]-1+j][goal[0]-1+i].text = 'G'
            
    with open("realtimemap.txt","w") as text_file_sim:
        text_file_sim.write(print_alt_descriptor())
##    if imgstr == '':
##        mail.write('AND|'+ print_descriptor() + ';' + str(robot.coord[0]) + ';' + str(robot.coord[1]) + ';' + robot.face)
##    else:
##        print('imgstr: ',end='')
##        print(imgstr)
##        mail.write('AND|'+ print_descriptor() + ';' + str(robot.coord[0]) + ';' + str(robot.coord[1]) + ';' + robot.face + ';' + imgstr)
    if len(sensedata) == 6:
        if sensedata[5] == 7:
            return
    else:
        color.write('Robot not ready','KEYWORD')
        #time.sleep(0.5)
        return

def step_eval(x, y, g):
    return (abs(g[0]-x) + abs(g[1]-y))

def h_eval(x, y, h):
    hor = abs(x-h[0])
    ver = abs(y-h[1])
    if x == h[0]:
        return(hor)
    elif y == h[1]:
        return(ver)
    else:
        return math.sqrt(hor**2 + ver**2)

def spaces(robot):
    wall = False
    x = robot.coord[0]
    y = robot.coord[1]
    count = 0
    if robot.face == 'W':
        while not wall:
            for i in range(3):
                 if mdp_map[y+count][x-1+i].text == 'W' or not mdp_map[y+count][x-1+i].explored:
                     return count
            count += 1
    elif robot.face == 'A':
        while not wall:
            for i in range(3):
                 if mdp_map[y-1+i][x-count].text == 'W' or not mdp_map[y-1+i][x-count].explored:
                     return count
            count += 1
    elif robot.face == 'S':
        while not wall:
            for i in range(3):
                 if mdp_map[y-count][x-1+i].text == 'W' or not mdp_map[y-count][x-1+i].explored:
                     return count
            count += 1
    elif robot.face == 'D':
        while not wall:
            for i in range(3):
                 if mdp_map[y-1+i][x+count].text == 'W' or not mdp_map[y-1+i][x+count].explored:
                     return count
            count += 1

def explore(robot):
    path = []
    steps = 0
    right_wall_before = False
    goal_explored = False
    if robot.coord == start:
        start_explored = True
    else:
        start_explored = False
    #first = True
    cal_count = 6
    mail.write('ARD|.')
    msg = mail.read('ARD')
    robot_sense(robot, msg)
    #right wall hug : ensure wall right is True as much as possible
    while robot.coord != start or (robot.coord == start and goal_explored == False):
##         or not(start_explored and goal_explored)
##        if steps == time_limit:
##            print('Time up!')
##            fastest_path(robot, robot.coord, start)
##            return
##        elif explore_percent() == explore_limit:
##            print('Explore limit reached!')
##            fastest_path(robot, robot.coord, start)
##            return
##        if goal_explored and start_explored and robot.coord != start:
##            print('Goal + Start explored; Proceeding back to start.')
##            fastest_path(robot, robot.coord, start)
##            return
##        print(steps)
        if robot.coord == goal:
            goal_explored = True
            print('Goal explored. Calibrating...')
            if robot_cornered(robot):
                mail.write('ARD|B')
                mail.read('ARD')
                cal_count = 6
        if robot.coord == start:
            start_explored = True
        if cal_count == 0:
            if robot_cornered(robot) or robot_rightwall(robot): 
##                #calibrate
##                mail.write('ARD|V')
##                mail.read('ARD')
##            elif robot_rightwall(robot):
                mail.write('ARD|B')
                mail.read('ARD')
            cal_count = 6
        #If right wall is the edge of the arena, activate cruise
##        if not robot.second_ahead:
##            if robot.face == 'W':
##                if oob(robot.coord[0]+2, robot.coord[1]) or mdp_map[robot.coord[1]+1][robot.coord[0]+2].text == 'W':
##                    cruise(robot)
##            elif robot.face == 'A':
##                if oob(robot.coord[0], robot.coord[1]+2) or mdp_map[robot.coord[1]+2][robot.coord[0]-1].text == 'W':
##                    cruise(robot)
##            elif robot.face == 'S':
##                if oob(robot.coord[0]-2, robot.coord[1]) or mdp_map[robot.coord[1]-1][robot.coord[0]-2].text == 'W':
##                    cruise(robot)
##            elif robot.face == 'D':
##                if oob(robot.coord[0], robot.coord[1]-2) or mdp_map[robot.coord[1]-2][robot.coord[0]+1].text == 'W':
##                    cruise(robot)
        elif robot.wall_right and not robot.wall_ahead:
##            print('WR&~WA')
##            if (not robot.second_ahead) and robot.movetwo:
##                robot_forward(robot, text='E')
##                #robot.movetwo = False
##            else:
            robot_forward(robot)
            right_wall_before = True
            path.append(robot.face)
        elif robot.wall_right and robot.wall_ahead:
            print('WR&WA')
            robot_left(robot)
            right_wall_before = True
        elif robot.wall_ahead and not right_wall_before:
            print('WA&~RB')
            robot_left(robot)
            right_wall_before = False
        elif robot.wall_ahead and right_wall_before:
            print('WA&RB')
            robot_right(robot)
            right_wall_before = False
        elif not robot.wall_right and right_wall_before:
            print('~WR&RB')
            robot_right(robot)
            right_wall_before = False
        else:
            print('NONE')
            robot_forward(robot)
            right_wall_before = False
            path.append(robot.face)
        steps += 1
        cal_count -= 1
        with open("realtimemap.txt","w") as text_file_sim:
            text_file_sim.write(print_alt_descriptor())

##    while not fully_explored():
##        print('Begin relocation routine.')
##        print(robot.coord)
##        fastest_path(robot, robot.coord, to_explore(robot.coord), True)
##        mail.write('ARD|.')
##        msg = mail.read('ARD')
##        if 'UP' in msg:
##            print('UP')
##            robot_sense(robot, msg[1], True)
##        else:
##            robot_sense(robot, msg)
    if robot.coord != start:
        fastest_path(robot, robot.coord, start)
    and_write(robot)


def fastest_path(robot, begin, end, explore=False):
    reset_fpath()
    #start from goal
    #formulate a path backwards towards goal
    #reverse the solution
    #!frontier -> coord, step, hvalue
    eye = [end[0], end[1]]
    mdp_map[end[1]][end[0]].fval = 0
    frontier = []
    path = []
    pathstring = ''
    wflag = False
    ###killswitch = robot.killswitch
    ###eye acts as a pointer to branch possible next paths from
    ### Set fval to be 1 if the previous node is the end
    ### If not, look at previous node's fval and add to it
    ### Save the new fval into the cell
    while eye != begin:
        #right
        if eye[0]+2 in range(15) and mdp_map[eye[1]][eye[0]].fpath != 'D':
            for i in range(3):
                if (mdp_map[eye[1]-1+i][eye[0]+2].text == 'W' or not mdp_map[eye[1]-1+i][eye[0]+2].explored) and not wflag:
                    wflag = True
                    #print('D, wall')
            if not wflag and mdp_map[eye[1]][eye[0]+2].explored:
                #print(str(eye[0]+1) + str(eye[1]))
                fval = 0
                if eye == end or mdp_map[eye[1]][eye[0]].fpath == 'A':
                    #print('ding')
                    fval = mdp_map[eye[1]][eye[0]].fval + 1
                else:
                    fval = mdp_map[eye[1]][eye[0]].fval + 2
                if mdp_map[eye[1]][eye[0]+1].fval > fval or mdp_map[eye[1]][eye[0]+1].fval == 0:    
                    mdp_map[eye[1]][eye[0]+1].fpath = 'A'
                    mdp_map[eye[1]][eye[0]+1].fval = fval
                    #print('frontier len: ' + str(len(frontier)))
                    for i in range(len(frontier)):
                        #print('frontier item: ' + str(frontier[i].coord))
                        if frontier[i].coord == [eye[0]+1,eye[1]]:
                            #print('deleting ' + str(frontier[i].coord))
                            del frontier[i]
                            break
                    frontier.append(Frontier([eye[0]+1,eye[1]], fval, h_eval(eye[0]+1,eye[1],begin)))
                    #print('D, frontier')
                #print('fval: ' + str(fval))
            wflag = False
            
        #left    
        if eye[0]-2 in range(15) and mdp_map[eye[1]][eye[0]].fpath != 'A':
            for i in range(3):
                if (mdp_map[eye[1]-1+i][eye[0]-2].text == 'W' or not mdp_map[eye[1]-1+i][eye[0]-2].explored) and not wflag:
                    wflag = True
                    #print('A, wall')
            if not wflag and mdp_map[eye[1]][eye[0]-2].explored:
                #print(str(eye[0]-1) + str(eye[1]))
                fval = 0
                if eye == end or mdp_map[eye[1]][eye[0]].fpath == 'D':
                    #print('ding')
                    fval = mdp_map[eye[1]][eye[0]].fval + 1
                else:
                    fval = mdp_map[eye[1]][eye[0]].fval + 2
                if mdp_map[eye[1]][eye[0]-1].fval > fval or mdp_map[eye[1]][eye[0]-1].fval == 0:    
                    mdp_map[eye[1]][eye[0]-1].fpath = 'D'
                    mdp_map[eye[1]][eye[0]-1].fval = fval
                    #print('frontier len: ' + str(len(frontier)))
                    for i in range(len(frontier)):
                        #print('frontier item: ' + str(frontier[i].coord))
                        if frontier[i].coord == [eye[0]-1,eye[1]]:
                            #print('deleting ' + str(frontier[i].coord))
                            del frontier[i]
                            break
                    frontier.append(Frontier([eye[0]-1,eye[1]], fval, h_eval(eye[0]-1,eye[1],begin)))
                    #print('A, frontier')
                #print('fval: ' + str(fval))
                #print('A, frontier')
            wflag = False

        #up
        if eye[1]+2 in range(20) and mdp_map[eye[1]][eye[0]].fpath != 'W':
            for i in range(3):
                if (mdp_map[eye[1]+2][eye[0]-1+i].text == 'W' or not mdp_map[eye[1]+2][eye[0]-1+i].explored) and not wflag:
                    wflag = True
                    #print('W, wall')
            if not wflag and mdp_map[eye[1]+2][eye[0]].explored:
                #print(str(eye[0]) + str(eye[1]+1))
                fval = 0
                if eye == end or mdp_map[eye[1]][eye[0]].fpath == 'S':
                    #print('ding')
                    fval = mdp_map[eye[1]][eye[0]].fval + 1
                else:
                    fval = mdp_map[eye[1]][eye[0]].fval + 2
                if mdp_map[eye[1]+1][eye[0]].fval > fval or mdp_map[eye[1]+1][eye[0]].fval == 0:    
                    mdp_map[eye[1]+1][eye[0]].fpath = 'S'
                    mdp_map[eye[1]+1][eye[0]].fval = fval
                    #print('frontier len: ' + str(len(frontier)))
                    for i in range(len(frontier)):
                        #print('frontier item: ' + str(frontier[i].coord))
                        if frontier[i].coord == [eye[0],eye[1]+1]:
                            #print('deleting ' + str(frontier[i].coord))
                            del frontier[i]
                            break
                    frontier.append(Frontier([eye[0],eye[1]+1], fval, h_eval(eye[0],eye[1]+1,begin)))
                    #print('W, frontier')
                #print('fval: ' + str(fval))
                #print('W, frontier')
            wflag = False

        #down
        if eye[1]-2 in range(20) and mdp_map[eye[1]][eye[0]].fpath != 'S':
            for i in range(3):
                if (mdp_map[eye[1]-2][eye[0]-1+i].text == 'W' or not mdp_map[eye[1]-2][eye[0]-1+i].explored) and not wflag:
                    wflag = True
                    #print('S, wall')
            if not wflag and mdp_map[eye[1]-2][eye[0]].explored:
                #print(str(eye[0]) + str(eye[1]-1))
                fval = 0
                if eye == end or mdp_map[eye[1]][eye[0]].fpath == 'W':
                    #print('ding')
                    fval = mdp_map[eye[1]][eye[0]].fval + 1
                else:
                    fval = mdp_map[eye[1]][eye[0]].fval + 2
                if mdp_map[eye[1]-1][eye[0]].fval > fval or mdp_map[eye[1]-1][eye[0]].fval == 0:    
                    mdp_map[eye[1]-1][eye[0]].fpath = 'W'
                    mdp_map[eye[1]-1][eye[0]].fval = fval
                    #print('frontier len: ' + str(len(frontier)))
                    for i in range(len(frontier)):
                        #print('frontier item: ' + str(frontier[i].coord))
                        if frontier[i].coord == [eye[0],eye[1]-1]:
                            #print('deleting ' + str(frontier[i].coord))
                            del frontier[i]
                            break
                    frontier.append(Frontier([eye[0],eye[1]-1], fval, h_eval(eye[0],eye[1]-1,begin)))
                    #print('S, frontier')
                #print('fval: ' + str(fval))
                    
                #print('S, frontier')
            wflag = False
        minima = min(frontier, key=attrgetter('total'))
        eye = [minima.x, minima.y]

        for i in range(len(frontier)):
            if frontier[i].x == eye[0] and frontier[i].y == eye[1]:
                del frontier[i]
                break
    while eye != end:
        path.append(mdp_map[eye[1]][eye[0]].fpath)
        if mdp_map[eye[1]][eye[0]].fpath == 'W':
            eye[1] += 1
        elif mdp_map[eye[1]][eye[0]].fpath == 'A':
            eye[0] -= 1
        elif mdp_map[eye[1]][eye[0]].fpath == 'S':
            eye[1] -= 1
        elif mdp_map[eye[1]][eye[0]].fpath == 'D':
            eye[0] += 1

    #Craft pathstring by using a replica of the robot to run through the fpaths
    if robot.coord != begin:
        print("Where's my robot?")
    else:
        sim_robot = Robot([robot.coord[0], robot.coord[1]], robot.face)
        while sim_robot.coord != end:
            next_path = path[0]
            if sim_robot.face == next_path:
                pathstring += 'W'
                robot_forward(sim_robot, fast=True)
                path.pop(0)
            elif sim_robot.face == 'W':
                if next_path == 'A':
                    pathstring += 'A'
                    robot_left(sim_robot,fast=True)
                elif next_path == 'D':
                    pathstring += 'D'
                    robot_right(sim_robot,fast=True)
                elif next_path == 'S':
                    pathstring += 'DD'
                    robot_right(sim_robot,fast=True)
                    robot_right(sim_robot,fast=True)
            elif sim_robot.face == 'A':
                if next_path == 'S':
                    pathstring += 'A'
                    robot_left(sim_robot,fast=True)
                elif next_path == 'W':
                    pathstring += 'D'
                    robot_right(sim_robot,fast=True)
                elif next_path == 'D':
                    pathstring += 'DD'
                    robot_right(sim_robot,fast=True)
                    robot_right(sim_robot,fast=True)
            elif sim_robot.face == 'S':
                if next_path == 'D':
                    pathstring += 'A'
                    robot_left(sim_robot,fast=True)
                elif next_path == 'A':
                    pathstring += 'D'
                    robot_right(sim_robot,fast=True)
                elif next_path == 'W':
                    pathstring += 'DD'
                    robot_right(sim_robot,fast=True)
                    robot_right(sim_robot,fast=True)
            elif sim_robot.face == 'D':
                if next_path == 'W':
                    pathstring += 'A'
                    robot_left(sim_robot,fast=True)
                elif next_path == 'S':
                    pathstring += 'D'
                    robot_right(sim_robot,fast=True)
                elif next_path == 'A':
                    pathstring += 'DD'
                    robot_right(sim_robot,fast=True)
                    robot_right(sim_robot,fast=True)
    
    #convert to burst string and send to ARD
    pathstring = burst_mode(pathstring)
    #print('Pathstring: ',end='')
    #print(pathstring)
    #mail.write('AND|'+ print_descriptor() + ';' + str(robot.coord[0]) + ';' + str(robot.coord[1]) + ';' + robot.face)
    #mail.write('ARD|' + pathstring)

    #Wait for ACK at each step and update map + android accordingly
    for i in pathstring:
        mail.write('ARD|' + i)
        try:
            recv = mail.read('ARD')
        except:
            print('error in receiving ARD msg.')
        while 'ACK' not in recv[0]:
            try:
                recv = mail.read('ARD')
            except:
                print('error in receiving ARD msg.')
        if i == 'A':
            robot_left(robot, fast=True)
        elif i == 'D':
            robot_right(robot, fast=True)
        elif i == 'W':
            robot_forward(robot, fast=True)
        elif i == 'E':
            for i in range(2):
                robot_forward(robot, fast=True)
        elif i == 'R':
            for i in range(3):
                robot_forward(robot, fast=True)
        elif i == 'T':
            for i in range(4):
                robot_forward(robot, fast=True)
        elif i == 'Y':
            for i in range(5):
                robot_forward(robot, fast=True)
        elif i == 'U':
            for i in range(6):
                robot_forward(robot, fast=True)
        elif i == 'I':
            for i in range(7):
                robot_forward(robot, fast=True)
        elif i == 'O':
            for i in range(8):
                robot_forward(robot, fast=True)
        elif i == 'P':
            for i in range(9):
                robot_forward(robot, fast=True)
        elif i == 'Q':
            for i in range(10):
                robot_forward(robot, fast=True)
        x = robot.coord[0]
        y = robot.coord[1]
        if explore:
            #D
            if not oob(x+2, y) and not mdp_map[y][x+2].explored:
                if robot.face == 'W':
                    robot_right(robot)
                elif robot.face == 'A':
                    robot_right(robot)
                    robot_right(robot)
                elif robot.face == 'S':
                    robot_left(robot)
            #A
            elif not oob(x-2, y) and not mdp_map[y][x-2].explored:
                if robot.face == 'W':
                    robot_left(robot)
                elif robot.face == 'S':
                    robot_right(robot)
                elif robot.face == 'D':
                    robot_right(robot)
                    robot_right(robot)
            #W
            elif not oob(x, y+2) and not mdp_map[y+2][x].explored:
                if robot.face == 'A':
                    robot_right(robot)
                elif robot.face == 'S':
                    robot_right(robot)
                    robot_right(robot)
                elif robot.face == 'D':
                    robot_left(robot)
            #S
            elif not oob(x, y-2) and not mdp_map[y-2][x].explored:
                if robot.face == 'W':
                    robot_right(robot)
                    robot_right(robot)
                elif robot.face == 'A':
                    robot_left(robot)                    
                elif robot.face == 'D':
                    robot_right(robot)
        with open("realtimemap.txt","w") as text_file_sim:
            text_file_sim.write(print_alt_descriptor())
        mail.write('AND|'+ print_descriptor() + ';' + str(robot.coord[0]) + ';' + str(robot.coord[1]) + ';' + robot.face)

def cruise(robot):
    mail.write('ARD|C')
    while True:
        msg = mail.read('ARD')
        #print('cruise msg: ' + msg[0] + ',' + msg[1])
        if 'END-CC' in msg[0]:
            #robot_forward(robot, fast=True)
            #print('forward')
            robot_sense(robot, msg[1])
            #print('sensed')
            and_write(robot)
            #print('written')
            return
        elif 'ACK' in msg[0]:
            if 'F' in msg[0]:
                robot_forward(robot, fast=True)
                #print('forward')
                robot_sense(robot, msg[1])
                #print('sensed')
                and_write(robot)
            elif 'TL' in msg[0]:
                robot_left(robot, fast=True)
                #print('left')
                robot_sense(robot, msg[1])
                #print('sensed')
                and_write(robot)
            elif 'TR' in msg[0]:
                robot_right(robot, fast=True)
                #print('right')
                robot_sense(robot, msg[1])
                #print('sensed')
                and_write(robot)
        
            
def robot_forward(robot, fast=False, text='W'):
    if not fast:
        recv = False
        mail.write('ARD|' + text)
        time.sleep(0.1)
        and_write(robot)
        #print('Sent to ARD')
        while not recv:
            try:
                ack = mail.read('ARD')
                #print('ack: ' + ack[0] + ack[1])
                if 'ACK' in ack[0]:
                    #print('RECEIVED')
                    recv = True
                else:
                    color.write('ACK not in string. Rereading...', 'KEYWORD')
                    time.sleep(0.5)
            except:
                color.write('Read error.', 'COMMENT')
                time.sleep(0.5)
    if text == 'W':
        move = 1
    elif text == 'E':
        move = 2
    robotcoord = robot[0]
    origincoord = Coordinates(robotcoord)
    #print(robot[0])
    robotface = robot.face
    if robotface == 'W':
        if robotcoord[1]+1 in range(20) and robotcoord[1]+2 in range(20):
            robotcoord[1] += move
    elif robotface == 'A':
        if robotcoord[0]-1 in range(15) and robotcoord[0]-2 in range(15):
            robotcoord[0] -= move
    elif robotface == 'S':
        if robotcoord[1]-1 in range(20) and robotcoord[1]-2 in range(20):
            robotcoord[1] -= move
    elif robotface == 'D':
        if robotcoord[0]+1 in range(15) and robotcoord[0]+2 in range(15):
            robotcoord[0] += move
    else:
        print('?')
    #print('robot coord updated')
    #print(robotcoord)
    #update map
    for i in range(3):
        for j in range(3):
            coord1 = origincoord[0]-1+i
            coord2 = origincoord[1]-1+j
            if coord1 in range(start[0]+2) and coord2 in range(start[1]+2):
                update_map(coord1, coord2, 'S')
            elif origincoord[0]-1+i in range(goal[0]-1, goal[0]+2) and origincoord[1]-1+j in range(goal[1]-1,goal[1]+2):
                update_map(coord1, coord2, 'G')
            else:
                update_map(coord1, coord2, ' ')
    #print('map updated')
    #robot
    for i in range(3):
        for j in range(3):
            update_map(robotcoord[0]-1+i, robotcoord[1]-1+j, 'R')
    if robot.face == 'W':
        update_map(robotcoord[0], robotcoord[1]+1, 'H')
    elif robot.face == 'A':
        update_map(robotcoord[0]-1, robotcoord[1], 'H')
    elif robot.face == 'S':
        update_map(robotcoord[0], robotcoord[1]-1, 'H')
    elif robot.face == 'D':
        update_map(robotcoord[0]+1, robotcoord[1], 'H')
    #print('No problems here.')
    if not fast:
        if 'UP' in ack:
            print('UP')
            robot_sense(robot, ack[1], True)
        else:
            robot_sense(robot, ack[1])

##    if fast:
##        mail.write('AND|'+ print_descriptor() + ';' + str(robot.coord[0]) + ';' + str(robot.coord[1]) + ';' + robot.face)
        

def robot_left(robot, fast=False):
    if not fast:
        recv = False
        mail.write('ARD|A')
        time.sleep(0.1)
        and_write(robot)
        #print('Sent to ARD')
        while not recv:
            try:
                ack = mail.read('ARD')
                if 'ACK' in ack[0]:
                    recv = True
                else:
                    color.write('ACK not in string. Rereading...', 'KEYWORD')
                    time.sleep(0.5)
            except:
                color.write('Read error.', 'COMMENT')
                time.sleep(0.5)
    robotcoord = robot[0]
    if robot.face == 'W':
        robot.face = 'A'
    elif robot.face == 'A':
        robot.face = 'S'
    elif robot.face == 'S':
        robot.face = 'D'
    elif robot.face == 'D':
        robot.face = 'W'
    else:
        print('?')
    for i in range(3):
        for j in range(3):
            update_map(robotcoord[0]-1+i, robotcoord[1]-1+j, 'R')
    if robot.face == 'W':
        update_map(robotcoord[0], robotcoord[1]+1, 'H')
    elif robot.face == 'A':
        update_map(robotcoord[0]-1, robotcoord[1], 'H')
    elif robot.face == 'S':
        update_map(robotcoord[0], robotcoord[1]-1, 'H')
    elif robot.face == 'D':
        update_map(robotcoord[0]+1, robotcoord[1], 'H')
    if not fast:
        if 'UP' in ack:
            print('UP')
            print(ack[1])
            robot_sense(robot, ack[1], True)
        else:
            print(ack[1])
            robot_sense(robot, ack[1])

##    if fast:
##        mail.write('AND|'+ print_descriptor() + ';' + str(robot.coord[0]) + ';' + str(robot.coord[1]) + ';' + robot.face)

def robot_right(robot, fast=False):
    if not fast:
        recv = False
        mail.write('ARD|D')
        time.sleep(0.1)
        and_write(robot)
        #print('Sent to ARD')
        while not recv:
            try:
                ack = mail.read('ARD')
                if 'ACK' in ack[0]:
                    recv = True
                else:
                    color.write('ACK not in string. Rereading...', 'KEYWORD')
                    time.sleep(0.5)
            except:
                color.write('Read error.', 'COMMENT')
                time.sleep(0.5)
    robotcoord = robot[0]
    if robot.face == 'W':
        robot.face = 'D'
    elif robot.face == 'A':
        robot.face = 'W'
    elif robot.face == 'S':
        robot.face = 'A'
    elif robot.face == 'D':
        robot.face = 'S'
    else:
        print('?')
    for i in range(3):
        for j in range(3):
            update_map(robotcoord[0]-1+i, robotcoord[1]-1+j, 'R')
    if robot.face == 'W':
        update_map(robotcoord[0], robotcoord[1]+1, 'H')
    elif robot.face == 'A':
        update_map(robotcoord[0]-1, robotcoord[1], 'H')
    elif robot.face == 'S':
        update_map(robotcoord[0], robotcoord[1]-1, 'H')
    elif robot.face == 'D':
        update_map(robotcoord[0]+1, robotcoord[1], 'H')

    if not fast:
        if 'UP' in ack:
            print('UP')
            robot_sense(robot, ack[1], True)
        else:
            robot_sense(robot, ack[1])

##    if fast:
##        mail.write('AND|'+ print_descriptor() + ';' + str(robot.coord[0]) + ';' + str(robot.coord[1]) + ';' + robot.face)

if __name__ == "__main__":
    #main
    start = [1,1]
    goal = [13,18]
    illegals = [3,4,5,6,8]
    mail = Algo()
    mdp_map = []
    mdp_explore_map = []
    initialize_map()
    msg = ''
    rdata = []
    to_turn = ''
    explore_complete = False
    while True:
        try:
            if rdata == []:
                recv = False
                while not recv:
                    try:
                        print('Robot data, please')
                        rdata = mail.read('AND')
                        robot = Robot([int(rdata[0]), int(rdata[1])],rdata[2])
                        place_robot(robot.coord)
                        recv = True
                    except:
                        color.write('Robot initialization failed, rereading...','KEYWORD')
                recv = False
                while not recv:
                    try:
                        print('Waypoints, please.')
                        rdata = mail.read('AND')
                        waypoint = [int(rdata[0]), int(rdata[1])]
                        recv = True
                    except:
                        color.write('Waypoint initialization failed, rereading...','KEYWORD')
            if msg == '':
                msg = mail.read('AND')
            if msg == 'e':
                #mail.write('ARD|.')
                explore(robot)
                explore_complete = True
                print('Explore complete, awaiting fastest path...')
                recv = False
                while not recv:
                    msg = mail.read('AND')
                    if msg == 'f':
                        recv = True
                        if waypoint == start or waypoint == goal or not valid_space(waypoint[0],waypoint[1]):
                            fastest_path(robot, start, goal)
                        else:
                            fastest_path(robot, start, waypoint)
                            fastest_path(robot, waypoint, goal)
                        msg = ''
            elif msg == 'f' and explore_complete:
                if waypoint == start or waypoint == goal or not valid_space(waypoint[0],waypoint[1]):
                    fastest_path(robot, start, goal)
                else:
                    fastest_path(robot, start, waypoint)
                    fastest_path(robot, waypoint, end)
                msg = ''
##            elif msg == 'end':
##                print('Understood. Ending...')
##                exit()
            else:
                msg == ''
        except Exception as e:
            print(e)
            pass
